...
class ReviewController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $reviews = \App\Models\Review::where(function ($q) {
            $q->where('status', 'approved')
              ->orWhere('status', '1')
              ->orWhere('status', 'active')
              ->orWhere('status', true);
        })->with(['user', 'product'])->latest()->paginate($perPage);
    ...
    public function store(StoreReviewRequest $request): JsonResponse
    {
        ...
        if ($user) {
            ...
            if (empty($data['author_name']) || $data['author_name'] === 'Verified Customer') {
                $data['author_name'] = $user->name;
            }
        }

        if (empty($data['author_name'])) {
            $data['author_name'] = 'Guest Customer';
        }
        ...
        return $this->success(new ReviewResource($review->load(['user', 'product'])), 'Review submitted successfully', 201);
    }
