...
<div class="space-y-4 sm:space-y-6 max-w-4xl mx-auto pb-16 px-1 sm:px-0" x-data="serviceForm()">
    <!-- Header & Breadcrumbs -->
    <div class="space-y-1">
        <div class="flex flex-wrap items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 font-medium">
            ...
            <span>&gt;</span>
            <span class="text-slate-800 dark:text-slate-200 font-semibold">Add Service</span>
        </div>
        <div class="flex items-center justify-between">
            <h1 class="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">Add New Service</h1>
    ...
    <form action="{{ route('admin.services.store') }}" method="POST" enctype="multipart/form-data" @submit="syncQuillContent()" class="space-y-4 sm:space-y-6">
        @csrf
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-sm space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="space-y-1">
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" required placeholder="e.g. Doorstep Computer Repair" class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white focus:outline-none focus:border-emerald-500 font-semibold">
                </div>
                <div class="space-y-1">
                    <label class="block text-xs font-bold text-slate-700 dark:text-slate-300">Custom Slug (Optional)</label>
                    <input type="text" name="slug" value="{{ old('slug') }}" placeholder="auto-generated-from-title" class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white font-mono">
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-sm space-y-4">
            ...
            <div class="space-y-2">
                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300">Service Image (Select File from PC / JPG, PNG, WebP)</label>
                    ...
                    <template x-if="imageBase64">
                        <div class="flex flex-col sm:flex-row items-center gap-4">
                            ...
                            <div class="space-y-2 text-center sm:text-left">
                                <span class="text-xs font-bold text-slate-800 dark:text-slate-200 block">Image Selected Successfully</span>
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-sm space-y-4">
            ...
            <div class="space-y-1">
                ...
                <textarea name="short_description" rows="2" class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white" placeholder="Quick 1-2 sentence overview of what this service offers...">{{ old('short_description') }}</textarea>
            </div>

            <!-- Full Rich Text Editor -->
            <div class="space-y-1">
                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300">Full Service Details (Rich Text Content)</label>
                <div id="service-quill-editor" class="bg-white dark:bg-slate-950"></div>
                <textarea name="description" id="service-description-input" class="hidden">{{ old('description') }}</textarea>
            </div>
...
function serviceForm() {
    return {
        ...
        syncQuillContent() {
            if (serviceQuill) {
                document.getElementById('service-description-input').value = serviceQuill.root.innerHTML;
            }
        }
    };
}
...
document.addEventListener('DOMContentLoaded', function () {
    serviceQuill = new Quill('#service-quill-editor', {
        theme: 'snow',
        placeholder: 'Write full service details, process, guarantees, and terms here...',
        modules: {
            toolbar: [
    ...
    serviceQuill.on('text-change', function() {
        document.getElementById('service-description-input').value = serviceQuill.root.innerHTML;
    });
});
</script>
