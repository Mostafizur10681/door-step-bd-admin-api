...
<div class="max-w-2xl mx-auto space-y-6 pb-16">
    ...
    @php
        $rawValues = old('values', ['S', 'M', 'L', 'XL']);
        if (is_string($rawValues)) {
            $decoded = json_decode($rawValues, true);
            $initialValues = is_array($decoded) ? $decoded : [];
        } elseif (is_array($rawValues)) {
            $initialValues = $rawValues;
        } else {
            $initialValues = ['S', 'M', 'L', 'XL'];
        }
        $initialValues = array_values(array_filter(array_map(function($v) {
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-7 shadow-sm">
        <form method="POST" action="{{ route('admin.attributes.store') }}" class="space-y-5" x-data="attributeCreateForm()" @submit="beforeSubmit()">
            @csrf
