...
<div class="max-w-6xl mx-auto space-y-6 pb-24 px-2 sm:px-4" x-data="{
    ...
    menu_location: {{ Js::from($banner->menu_location ?: 'home_hero_slider') }},
    order: {{ (int) ($banner->order ?? 1) }},
    is_active: {{ $banner->is_active ? "'1'" : "'0'" }},
    imagePreview: {{ Js::from($banner->image ? (str_starts_with($banner->image, 'data:') || str_starts_with($banner->image, 'http') || str_starts_with($banner->image, '/') ? $banner->image : asset('storage/' . $banner->image)) : '/hero_honey.png') }},
    imageBase64: '',
    imageUrl: '',
    mobileImagePreview: {{ Js::from($banner->mobile_image ? (str_starts_with($banner->mobile_image, 'data:') || str_starts_with($banner->mobile_image, 'http') || str_starts_with($banner->mobile_image, '/') ? $banner->mobile_image : asset('storage/' . $banner->mobile_image)) : '') }},
    mobileImageBase64: '',
    mobileImageUrl: '',
    ...
    <form id="bannerEditForm" method="POST" action="{{ route('admin.banners.update', $banner->id) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400">
                    Update banner graphic artwork, optional promotional text overlay, and display placement.
                </p>
            </div>
            <div class="flex items-center gap-3">
                ...
                <button type="submit" class="inline-flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl shadow-md shadow-emerald-600/20 transition cursor-pointer">
        ...
        @if (isset($errors) && $errors->any())
            <div class="p-4 rounded-2xl bg-rose-50 dark:bg-rose-950/50 border border-rose-200 dark:border-rose-800 text-rose-800 dark:text-rose-200 text-xs space-y-1">
                <div class="font-bold flex items-center gap-1.5">
                    <svg class="w-4 h-4 text-rose-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-sm space-y-8">
            ...
            <!-- ── SECTION 1: BANNER ARTWORK & IMAGES ── -->
                    ...
                    <div class="p-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-3">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex-1 min-w-0 space-y-1">
                                ...
                                <p class="text-[10px] text-slate-400">Replace current desktop banner artwork</p>
                            </div>
                        </div>

                        <div class="pt-1">
                            <label class="block text-[11px] font-semibold text-slate-500 mb-1">Or Replace with Image URL:</label>
                            <input type="text" name="image_url" x-model="imageUrl" placeholder="https://example.com/banner.jpg" class="w-full px-3 py-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-mono text-slate-800 dark:text-slate-200 focus:outline-none focus:border-emerald-500">
                        </div>
                    </div>
