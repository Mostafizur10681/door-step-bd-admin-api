...
<div class="space-y-4 sm:space-y-6 max-w-5xl mx-auto pb-16 px-1 sm:px-0" x-data="blogForm()">
    <div class="space-y-1">
        <div class="flex flex-wrap items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 font-medium">
            ...
            <span>&gt;</span>
            <span class="text-slate-800 dark:text-slate-200 font-semibold">Add Article</span>
        </div>
        <h1 class="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">Add New Blog Post</h1>
    </div>

    <form action="{{ route('admin.blogs.store') }}" method="POST" @submit="syncQuillContent()" class="space-y-4 sm:space-y-6">
        @csrf
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-sm space-y-4">
            ...
            <div class="space-y-1">
                ...
                <input type="text" name="title" required placeholder="e.g. Top 10 Must-Have Tech Gadgets in 2026" class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white focus:outline-none focus:border-emerald-500 font-semibold">
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div class="space-y-1">
                    ...
                    <input type="text" name="author_name" value="{{ auth()->user()->name ?? 'Admin' }}" class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white font-semibold">
                </div>
                <div class="space-y-1">
                    ...
                    <template x-if="imageBase64">
                        <div class="flex flex-col sm:flex-row items-center gap-4">
                            ...
                            <div class="space-y-2 text-center sm:text-left">
                                <div class="text-xs font-bold text-slate-800 dark:text-slate-200">Image Loaded Successfully (Base64 Format)</div>
            ...
            <div class="space-y-1">
                ...
                <textarea name="short_description" rows="2" class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white" placeholder="Brief 1-2 sentence overview of the article..."></textarea>
            </div>
            ...
            <div class="space-y-1">
                ...
                <div id="quill-editor" class="bg-white dark:bg-slate-950"></div>
                <textarea name="content" id="blog-content-input" class="hidden" required></textarea>
            </div>

            <div class="pt-2 flex items-center gap-2">
                <input type="checkbox" name="featured" id="featured" value="1" class="h-4 w-4 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500">
                <label for="featured" class="text-xs font-bold text-slate-700 dark:text-slate-300">Mark as Featured Story (Highlight on home page)</label>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-sm space-y-4">
            ...
            <div class="space-y-1">
                ...
                <input type="text" name="meta_title" placeholder="SEO Title tag" class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white">
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="space-y-1">
                    <label class="block text-xs font-bold text-slate-700 dark:text-slate-300">Meta Description</label>
                    <textarea name="meta_description" rows="2" class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white"></textarea>
                </div>
                <div class="space-y-1">
                    <label class="block text-xs font-bold text-slate-700 dark:text-slate-300">Meta Keywords</label>
                    <textarea name="meta_keywords" rows="2" placeholder="electronics, gadgets, tech 2026" class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white"></textarea>
                </div>
            </div>
            ...
            <div class="pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-col-reverse sm:flex-row sm:justify-end gap-2.5 sm:gap-3">
                ...
                <button type="submit" class="w-full sm:w-auto text-center px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold shadow-md shadow-emerald-600/20">Publish Article</button>
            </div>
        </div>
    </form>
</div>
